...
class MotionTrack(MQTTClient):
    ...
    def __calc_servo(self, servo: object, bbox: dict, camera: str) -> int:
        ...
        # If the camera is the head camera, set the correct FOV
        ...
        if camera in (CameraEnum.CAMERA_LEFT.value, CameraEnum.CAMERA_RIGHT.value):
            self.logger.debug(
                f"Side camera calc is currently at {current} with an adjustment of {angle_adjustment} "
                f"before mounting correction of {mounting_angle} and a direction angle of {direction_factor}"
            )
